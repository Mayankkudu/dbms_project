...
SELECT
    ...
    pe.first_name || ' ' || pe.last_name AS patient_name,
...
ORDER BY 
    CASE ca.severity
        ...
        ELSE 4
    END, 
    ca.generated_at DESC;

CREATE OR REPLACE VIEW doctor_patient_view AS
SELECT DISTINCT
    d.doctor_id, dp.first_name || ' ' || dp.last_name AS doctor_name,
    pt.patient_id, pp.first_name || ' ' || pp.last_name AS patient_name,
    pt.current_status
FROM doctors d
